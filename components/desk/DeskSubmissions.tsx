...
import { articleToDeskSubmission } from "./desk-article-mappers";
import { submissionsNeedingAction, type DeskSubmissionItem } from "./desk-submission-types";
...
export function DeskSubmissions() {
  ...
  const [items, setItems] = useState<DeskSubmissionItem[]>([]);
  ...
  useEffect(() => {
    ...
    async function loadSubmissions() {
      ...
      if (result.error) {
        ...
        const mapped = (result.data ?? []).map(articleToDeskSubmission);
        setItems(mapped);
  ...
  const actionCount = submissionsNeedingAction(items);
  ...
  const active = items.find((s) => s.id === activeId) ?? pageItems[0];
  ...
  return (
    <section>
      ...
      {loadError ? (
        <div className={clsx("mb-2 font-robinhood text-[11px] text-desk-red", deskPaper.inkBody)}>{loadError}</div>
      ) : null}

      <div className={clsx("overflow-hidden rounded-md border", deskPaper.border)}>
        {loading ? (
          <div className={clsx("px-4 py-8 font-robinhood text-[12px]", deskPaper.inkMeta)}>Loading submissions…</div>
        ) : items.length === 0 ? (
          <DeskEmptyState title="No submissions in the pipeline." subtitle="Drafts and in-review stories will appear here." />
        ) : (
          <>
