...
export default function NewspaperPage({ searchParams }: { searchParams: Search }) {
  const raw = searchParams.story;
  const queryId = typeof raw === "string" ? raw : Array.isArray(raw) ? raw[0] : undefined;
  const selected = (queryId && getBriefingById(queryId)) || NEWSROOM_BRIEFINGS[0];
  const frontPage = NEWSROOM_BRIEFINGS.filter((story) => story.id !== selected.id);
            ...
            <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
              <div>
                ...
                <p className="mt-3 max-w-2xl text-pretty font-luxury-sans text-[0.98rem] leading-relaxed text-[#4b3b29] md:text-[1.05rem]">
                  A paper-of-record treatment for the same desk signals surfaced inside the home hero carousel. The
                  selected brief runs lead; the rest of the feed remains on the front page.
                </p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 text-right font-mono-hbm text-[9px] uppercase tracking-[0.16em] text-[#6a5843] md:grid-cols-3">
                <div>
                  ...
                  <p className="mt-1">{getBriefingUpdatedLabel(BRIEFING_UPDATED_AT)}</p>
                </div>
                <div>
                  <p className="text-[#9a8262]">Lead Story</p>
                  <p className="mt-1">Story {selected.id}</p>
                </div>
                <div className="col-span-2 md:col-span-1">
                  <p className="text-[#9a8262]">Sections</p>
                  <p className="mt-1">Markets · Infrastructure · Governance</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <section className="border-t border-[#bca882]/35 px-6 py-8 lg:px-10">
            <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
              ...
              <p className="max-w-2xl text-sm leading-relaxed text-[#5b4935]">
                The same four stories that drive the hero carousel also populate the lead package here. Select a story
                above to make it the front-page brief.
              </p>
            </div>

            <div className="mt-8 grid gap-6 md:grid-cols-2 xl:grid-cols-4">
              {NEWSROOM_BRIEFINGS.map((story) => (
                <Link
                  key={story.id}
                  href={`/newspaper?story=${story.id}`}
